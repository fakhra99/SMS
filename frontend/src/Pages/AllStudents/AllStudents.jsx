    ...
    const fetchStudents = async () => {
      try {
        const response = await axios.get(
          "http://localhost:4041/api/allStudents"
        );
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:4041/api/update/${currentStudent._id}`,
        currentStudent
      );
      setStudents(
        students.map((student) =>
          student._id === currentStudent._id ? response.data.student : student
        )
      );
      setIsEditing(false);
          ...
          {students.map((student, index) => (
            <tr key={student._id} className="hover:bg-gray-100">
              ...
              <td className="px-4 py-2">
                <img
                  src={`http://localhost:4041/upload/${student.Image}`}
                  alt="Student"
